  ...
  return (
    <div>
      ...
      <div className="lg:basis-1/4">
                    {newCategories.map((i)=>(
                        <div  className={`tab bg-gray-100 shadow-md rounded border-gray-800  border-b-[1px] border-solid px-4 py-2 text-black text-sm flex items-center justify-between ${activeCategory === i.id ? 'active bg-primary' : ''}`}  onClick={() => setActiveCategory(i.id)}>{i.name}<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor" className="size-4">
   ...
   <div className="animate-spin rounded-full h-10 w-10 border-b-2 border-black"></div>
              ...
              <div key={product?.id} className="flex items-center justify-between p-5 rounded shadow-lg bg-gray-50 border-b-2 border-solid border-gray-800">
